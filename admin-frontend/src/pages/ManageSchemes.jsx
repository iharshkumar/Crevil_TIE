    ...
    try {
      const res = await fetch('http://localhost:5001/api/admin/schemes', {
        ...
        body: JSON.stringify({
          ...
          eligibilityCriteria: { maxIncomeLimit: Number(formData.maxIncome) || 250000, minAge: 0, maxAge: 100, genderAllowed: ['all'], residenceState: 'All' }
        })
      });
      const resData = await res.json();
      if (resData.success) {
        setDbStatus('Scheme successfully saved & pushed to MongoDB database!');
        loadSchemesFromDB();
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Push official scheme links, required documents, and benefits into MongoDB</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl font-medium shadow-xs transition-all cursor-pointer"
        >
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500"
          />
        </div>
        <button className="flex items-center gap-2 px-3.5 py-2 bg-slate-100 border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors cursor-pointer">
            ...
            {filteredSchemes.map((scheme) => (
              <tr key={scheme.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="px-6 py-4 space-y-1">
                  <div className="font-bold text-slate-900 text-base">{scheme.title}</div>
                  <div className="text-xs text-slate-500">{scheme.ministry}</div>
                  {scheme.officialUrl && (
                    <a
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1 max-w-xs">
                    {scheme.documentsRequired?.map((doc, idx) => (
                      <span key={idx} className="bg-slate-100 border border-slate-200 text-slate-700 text-[10px] px-2 py-0.5 rounded font-medium">
                        ✓ {doc}
                      </span>
                    ))}
                  </div>
                ...
                <td className="px-6 py-4 text-right space-x-2">
                  <button className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-lg cursor-pointer">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-xl p-6 space-y-5 my-8 shadow-xl text-slate-900">
            ...
            <form onSubmit={handleAddScheme} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:bg-white focus:border-indigo-500 focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-indigo-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:bg-white focus:border-indigo-500 focus:outline-none"
                />
                <span className="text-[11px] text-slate-500 mt-1 block">Direct URL where beneficiaries can apply online</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Max Annual Income Limit (₹)</label>
                  <input
                    type="number"
                    placeholder="250000"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Ministry Name</label>
                <input
                  type="text"
              ...
              <div className="space-y-2 border-t border-slate-100 pt-3">
                <label className="block text-xs font-semibold text-emerald-700 flex items-center gap-1.5">
                  <FileCheck className="w-4 h-4" />
                  Required Documents (Select from Dropdown List) *
                </label>
                <div className="grid grid-cols-2 gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200 max-h-48 overflow-y-auto text-xs">
                ...
                <p className="text-[11px] text-slate-500">Selected: {formData.documentsRequired.join(', ') || 'None selected'}</p>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Key Benefits Summary</label>
                <textarea
                  rows="2"
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
                <button
                  ...
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-sm font-medium text-slate-700 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl text-sm font-bold text-white shadow-xs cursor-pointer"
                >
